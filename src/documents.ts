...
const limitDigits = (value: InputValue, limit: number): string => onlyDigits(value).slice(0, limit);
...
export const isCPFValid = (value: InputValue): boolean => {
	...
	const calculateDigit = (baseDigits: string, factor: number): number => {
	...
	return (
		digits ===
		`${base}${calculateDigit(base, 10)}${calculateDigit(`${base}${calculateDigit(base, 10)}`, 11)}`
	);
};
...
export const isCNPJValid = (value: InputValue): boolean => {
	...
	const calculateDigit = (baseDigits: string, weights: readonly number[]): number => {
		const sum = [...baseDigits].reduce(
